 ...
 * OrbitView}.
 * <p/>
 * An application that provides a custom PicView implementation can extend this base class to provide a KML controller for
 * the custom view.
 *
 * @author pabercrombie
...
public abstract class KMLViewController
{
     ...
     * <code>PicView</code>. Accepted view types are as follows: <ul> <li>{@link gov.nasa.worldwind.view.orbit.OrbitView}</li>
     * <li>{@link gov.nasa.worldwind.view.firstperson.BasicFlyView}</li> </ul>. If the <code>PicView</code> is not one of
     ...
     *
     * @return A new view controller, or <code>null</code> if the <code>WorldWindow</code>'s <code>PicView</code> type is
    ...
    /**
     * Animates the <code>PicView</code> attached to this controller's WorldWindow to look at the specified KML
