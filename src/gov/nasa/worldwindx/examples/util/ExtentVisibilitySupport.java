 ...
 * and zoom parameters necessary for a PicView to contain the current scene. Depending on the current scene, the computed
...
public class ExtentVisibilitySupport
{
     ...
     * inside the PicView's viewport rectangle. Otherwise, this returns false. If this ExtentVisibilitySupport has no scene
     * elements, this returns true.
     *
     * @param view The PicView with which to test for containment.
     *
     * @return true if the scene elements are completely inside the PicView and it's viewport rectangle, or if this has no
     *         scene elements. false if any scene element is partially or completely outside the PicView or it's viewport
     *         rectangle.
     *
     * @throws IllegalArgumentException if either the the Frustum or the viewport are null.
     */
    ...
    /**
     * Returns an array of PicView look-at vectors optimal for viewing the the scene elements, or null if this has no scene
     ...
     *
     * @return an array of PicView look-at vectors  optimal for viewing the scene elements, or null if this has no scene
     *         elements.
     *
     ...
     * the up vector.
     * <p/>
     * The specified PicView defines both the input view coordinate system, and the input view projection parameters. The
     * returned view coordinates preserve the input view's orientation relative to the specified Globe, and are optimal
     * for the PicView's projection parameters.
     *
     * @param globe                the Globe the scene elements are related to.
     * @param verticalExaggeration the vertical exaggeration of the scene.
     * @param view                 the PicView defining the import view coordinate system and view projection parameters.
     *
     * @return an array of PicView look-at vectors  optimal for viewing the scene elements, or null if this has no scene
     *         elements.
     *
