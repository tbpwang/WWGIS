...
/**
 * The <code>PicView</code> interface provides a coordinate transformation from model coordinates to eye coordinates. This
 * follows the OpenGL convention of a right-handed coordinate system with the origin at the eye point and looking down
 * the negative Z axis. <code>PicView</code> also provides a transformation from eye coordinates to screen coordinates,
 * following the OpenGL convention of an origin in the lower left hand screen corner.
 * <p/>
 * Most of the accessor and computation methods on <code>PicView</code> will use viewing state computed in the last call to
 ...
 *
 * @author Paul Collins
 * @version $Id: PicView.java 1171 2013-02-11 21:45:02Z dcollins $
 * @see gov.nasa.worldwind.view.orbit.OrbitView
 */
public interface View extends WWObject, Restorable
{

    final String VIEW_STOPPED = "gov.nasa.worldwind.PicView.ViewStopped";

    /** Stops any movement associated with this <code>PicView</code>. */
    void stopMovement();
    ...
    /**
     * Returns this PicView's current roll.
     *
     ...
     * {@link #getEyePoint}, getCurrentEyePoint will return the PicView's immediate position.
     *
     * @return Vec4 of the eye.
     */
    ...
    /**
     * Returns the near clipping plane distance, in eye coordinates.  Implementations of the <code>PicView</code> interface
    ...
    /**
     * Returns the far clipping plane distance, in eye coordinates. Implementations of the <code>PicView</code> interface
     * are not required to have a method for setting the near and far distance. Applications that need to control the
     * near and far clipping distances can derive from {@link gov.nasa.worldwind.view.orbit.BasicOrbitView} or {@link
     * gov.nasa.worldwind.view.firstperson.BasicFlyView}
     *
     ...
     * is constructed using the projection parameters specific to each implementation of <code>PicView</code>. The method
    ...
    /**
     * Calculates and applies this <code>PicView's</code> internal state to the graphics context in the specified
     ...
     *
     * @param dc the current World Wind DrawContext on which <code>PicView</code> will apply its state.
     *
     ...
     * (x, y) on the <code>PicView's</code> projection plane (or after projection into model space).
     *
     * @param x the horizontal coordinate originating from the left side of <code>PicView's</code> projection plane.
     * @param y the vertical coordinate originating from the top of <code>PicView's</code> projection plane.
     *
     * @return a line beginning at the <code>PicView's</code> eye point and passing through (x, y) transformed into model
     *         space.
     */
     ...
     * <code>PicView.apply()</code>. <code>PicView.getGlobe()</code> returns null if called before the first invocation of
     * <code>PicView.apply()</code>.
     *
     * @return the globe being rendered by this view, or null before the first invocation of <code>PicView.apply()</code>.
     */
    Globe getGlobe();

    /**
     * Gets the <code>ViewInputHandler</code> being used to map input events to <code>PicView</code> controls.
     *
     * @return the <code>ViewInputHandler</code> being used to map input events to <code>PicView</code> controls.
     */
    ViewInputHandler getViewInputHandler();

    /** Stops any animations that are active in this <code>PicView</code> */
    void stopAnimations();

    /**
     * Animate to the specified position.  The implementation is expected to animate the <code>PicView</code> to look at
    ...
    /**
     * Determine if there are any animations active in the <code>PicView</code>.
     *
    ...
    /**
     * Copy the state of the given <code>PicView</code>.
     *
     * @param view The <code>PicView</code> whose state is to be copied.
     */
    void copyViewState(View view);

    /**
     * Add an animator to the <code>PicView</code>. This method does not start the {@link
